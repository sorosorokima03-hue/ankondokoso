...
function createGameScene(param) {
    ...
    scene.onLoad.add(() => {
        ...
        const font = new g.DynamicFont({
            game: g.game, fontFamily: "sans-serif", size: 32,
            strokeWidth: 5, strokeColor: "black", fontColor: "white", fontWeight: "bold"
        });
        const simpleFont = new g.DynamicFont({
            game: g.game, fontFamily: "sans-serif", size: 24, fontColor: "black"
        });
        const arrowFont = new g.DynamicFont({
            game: g.game, fontFamily: "sans-serif", size: 60,
            strokeWidth: 4, strokeColor: "white", fontColor: "white", fontWeight: "bold"
        });

        // --- レイヤー構成 ---
        const bgLayer = new g.E({ scene: scene });
        scene.append(bgLayer);
        const mapLayer = new g.E({ scene: scene });
        scene.append(mapLayer);
        const uiLayer = new g.E({ scene: scene });
        scene.append(uiLayer);
        const messageLayer = new g.E({ scene: scene });
        scene.append(messageLayer);

        // --- 背景 (予備色) ---
        const fallbackBg = new g.FilledRect({
            scene: scene, width: g.game.width, height: g.game.height, cssColor: "#e0e0e0"
        });
        bgLayer.append(fallbackBg);

        // --- スタート画面 ---
        const startScreenGroup = new g.E({ scene: scene });
        messageLayer.append(startScreenGroup);

        const pushStartMessage = new g.Label({
            scene: scene, text: "PUSH START", font: font, fontSize: 80,
            x: 0, y: g.game.height / 2 - 40, width: g.game.width, textAlign: "center", cssColor: "red"
        });
        ...
        // --- UI要素 ---
        let timeGaugeBg, timeGaugeBar, timeLabel, tileCountLabel, scoreLabel,
            undoBtnGroup, undoBg, undoText,
            bgmVolLabel, bgmVolBg, bgmVolBar, bgmVolTouch,
            seVolLabel, seVolBg, seVolBar, seVolTouch;
        ...
        const playSe = () => {
            if (seAudioAsset) {
                try { seAudioAsset.play().changeVolume(seVolume); } catch (e) { }
            }
        };

        const initUI = () => {
            const gaugeHeight = 40;
            timeGaugeBg = new g.FilledRect({ scene: scene, width: g.game.width, height: gaugeHeight, cssColor: "#333333", x: 0, y: 0 });
            uiLayer.append(timeGaugeBg);
            timeGaugeBar = new g.FilledRect({ scene: scene, width: g.game.width, height: gaugeHeight, cssColor: "#00ff00", x: 0, y: 0 });
            uiLayer.append(timeGaugeBar);
            timeLabel = new g.Label({ scene: scene, text: "TIME: " + GAME_TIME_LIMIT, font: font, fontSize: 32, x: 20, y: 4 });
            uiLayer.append(timeLabel);
            tileCountLabel = new g.Label({ scene: scene, text: "TILES: 00", font: font, fontSize: 32, x: 20, y: gaugeHeight + 4 });
            uiLayer.append(tileCountLabel);
            scoreLabel = new g.Label({ scene: scene, text: "SCORE: 0", font: font, fontSize: 32, x: g.game.width - 300, y: 4 });
            uiLayer.append(scoreLabel);

            const sideBarX = 1180;
            undoBtnGroup = new g.E({ scene: scene, x: sideBarX, y: 120, width: 80, height: 120, touchable: true });
            undoBg = new g.FilledRect({ scene: scene, width: 80, height: 120, cssColor: "#4488ff", rx: 8 });
            undoText = new g.Label({ scene: scene, text: "←", font: arrowFont, fontSize: 60, x: 8, y: 25, width: 80, textAlign: "center" });
            ...
            undoBtnGroup.pointDown.add(() => {
                ...
                undoBg.cssColor = "#2255aa"; undoBg.modified();
            });
            undoBtnGroup.pointUp.add(() => {
                if (!isGameStarted) return;
                undoBg.cssColor = "#4488ff"; undoBg.modified();
                executeUndo();
            });

            const volAreaY = 320, volHeight = 250, volBarW = 30, volGap = 50;
            const bgmBarX = sideBarX - 10, seBarX = sideBarX + volGap;
            const createVolBar = (name, x, color) => {
                const label = new g.Label({ scene: scene, text: name, font: simpleFont, fontSize: 20, x: x, y: volAreaY - 30, textAlign: "center", width: volBarW });
                uiLayer.append(label);
                const bg = new g.FilledRect({ scene: scene, x: x, y: volAreaY, width: volBarW, height: volHeight, cssColor: "#cccccc" });
                uiLayer.append(bg);
                const bar = new g.FilledRect({ scene: scene, x: x, y: volAreaY + volHeight, width: volBarW, height: 0, cssColor: color });
                uiLayer.append(bar);
                const touch = new g.FilledRect({ scene: scene, x: x - 10, y: volAreaY - 20, width: volBarW + 20, height: volHeight + 40, cssColor: "black", opacity: 0, touchable: true });
                uiLayer.append(touch);
                return { label, bg, bar, touch };
            };
            const bgmObj = createVolBar("BGM", bgmBarX, "#00aaff");
            bgmVolLabel = bgmObj.label; bgmVolBg = bgmObj.bg; bgmVolBar = bgmObj.bar; bgmVolTouch = bgmObj.touch;
            const seObj = createVolBar("SE", seBarX, "#ffaa00");
            seVolLabel = seObj.label; seVolBg = seObj.bg; seVolBar = seObj.bar; seVolTouch = seObj.touch;
            ...
            bgmVolTouch.pointDown.add((ev) => {
                ...
                currentBGMVolume = r; g.game.audio.music.volume = r; updateVolume(bgmVolBar, r);
            ...
            seVolTouch.pointDown.add((ev) => {
                ...
                g.game.audio.sound.volume = r; updateVolume(seVolBar, r);
            });
            seVolTouch.pointMove.add((ev) => {
                const r = Math.max(0, Math.min(1, 1.0 - ((ev.point.y + (seVolTouch.y - volAreaY)) / volHeight)));
                g.game.audio.sound.volume = r; updateVolume(seVolBar, r);
            });
            g.game.audio.music.volume = currentBGMVolume; updateVolume(bgmVolBar, currentBGMVolume);
            g.game.audio.sound.volume = seVolume; updateVolume(seVolBar, seVolume);
        };

        // --- 牌クラス ---
        class Tile extends g.E {
            constructor(id, gx, gy, gz) {
                super({ scene: scene, width: TILE_W, height: TILE_H });
                this.tileId = id;
                ...
                this.shadow = new g.FilledRect({ scene: scene, width: TILE_W, height: TILE_H, cssColor: "#000000", opacity: 0.3, x: 6, y: 6 });
                this.append(this.shadow);

                if (kyoryuAsset) {
                    this.sprite = new g.FrameSprite({
                        scene: scene, src: kyoryuAsset, width: TILE_W, height: TILE_H, srcWidth: SRC_TILE_W, srcHeight: SRC_TILE_H, frames: [this.tileId], x: 0, y: 0
                    });
                    this.append(this.sprite);
                } else {
                    this.dummyRect = new g.FilledRect({ scene: scene, width: TILE_W, height: TILE_H, cssColor: "gray" });
                    this.append(this.dummyRect);
                }
                this.highlight = new g.FilledRect({ scene: scene, width: TILE_W, height: TILE_H, cssColor: "#ffff00", opacity: 0, hidden: false });
            ...
            updateId(newId) { this.tileId = newId; if (this.sprite) { this.sprite.frames = [newId]; this.sprite.modified(); } }
            setSelected(value) { this.isSelected = value; }
            revive() { this.active = true; this.show(); this.touchable = true; this.isSelected = false; }
            remove() { this.active = false; this.hide(); this.touchable = false; }
            destroyChildren() { this.children?.forEach(c => { if (c && !c.destroyed()) c.destroy(); }); }
        }
        ...
        function assignSolvableIds(skeleton) {
            ...
            const finalLayout = [];
            pairs.forEach((p, i) => {
                finalLayout.push({ gx: p[0].gx, gy: p[0].gy, gz: p[0].gz, tileId: ids[i] });
                finalLayout.push({ gx: p[1].gx, gy: p[1].gy, gz: p[1].gz, tileId: ids[i] });
            });
            return finalLayout;
        }
        ...
        function createSkeleton(isInitial) {
            ...
            for (let y = 1; y < MAX_GRID_Y; y++) {
                for (let x = 2; x < MAX_GRID_X - 1; x++) {
                    ...
                    if (g.game.random.get(0, 100) < density) skeleton.push({ gx: x, gy: y, gz: 0 });
                }
            }
            let safety = 0;
            while (skeleton.length < TARGET_TILE_COUNT && safety < 6000) {
                ...
                if (base.gz < MAX_SHUFFLE_HEIGHT - 1) {
                    if (!skeleton.some(s => s.gx === base.gx && s.gy === base.gy && s.gz === base.gz + 1)) {
                        skeleton.push({ gx: base.gx, gy: base.gy, gz: base.gz + 1 });
                        continue;
                    }
                }
                if (base.gz === 0) {
                    const neighbors = [{ x: 1, y: 0 }, { x: -1, y: 0 }, { x: 0, y: 1 }, { x: 0, y: -1 }];
                    const empty = neighbors.map(n => ({ gx: base.gx + n.x, gy: base.gy + n.y, gz: 0 }))
        ...
        function checkDeadlockAndReshuffle() {
            ...
            for (let i = 0; i < removables.length; i++) {
                for (let j = i + 1; j < removables.length; j++) {
                    if (removables[i].tileId === removables[j].tileId) { hasPair = true; break; }
                }
                if (hasPair) break;
            }
            ...
            if (!hasPair) {
                const msg = new g.Label({
                    scene: scene, text: "詰み！牌の種類をシャッフルします",
                    font: font, fontSize: 48, x: 0, y: g.game.height / 2 - 50, width: g.game.width, textAlign: "center"
                });
                messageLayer.append(msg);
                scene.setTimeout(() => {
                    const skeleton = active.map(t => ({ gx: t.gx, gy: t.gy, gz: t.gz }));
                    ...
                    if (firstSelectedTile) { firstSelectedTile.setSelected(false); firstSelectedTile = null; }
                    history.length = 0;
                    msg.destroy();
                }, 1500);
            }
        ...
        function onTileClicked(tile) {
            ...
            if (firstSelectedTile === null) {
                ...
                if (firstSelectedTile === tile) {
                    ...
                    scoreLabel.text = "SCORE: " + g.game.vars.gameState.score;
                    scoreLabel.invalidate();
                    history.push({ t1: firstSelectedTile, t2: tile, score: gain });
            ...
            if (loadingStep > 0) {
                switch (loadingStep) {
                    case 1: // アセット変数確保 & UI初期化
                        ...
                        if (backAsset) {
                            const backgroundSprite = new g.Sprite({ scene: scene, src: backAsset, width: g.game.width, height: g.game.height });
